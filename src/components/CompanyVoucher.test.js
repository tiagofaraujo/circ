...
test('requires eligible external onsite selection without extras and explicit confirmation', async () => {
  ...
  fireEvent.change(screen.getByLabelText('Código da empresa'), { target: { value: 'a'.repeat(32) } });
  ...
  expect(await screen.findByText('Inscrição confirmada e paga pela empresa.')).toBeInTheDocument();
...
test('failed redemption keeps code for retry and does not claim confirmation', async () => {
  ...
  render(<CompanyVoucher eligible />); fireEvent.change(screen.getByLabelText('Código da empresa'), { target: { value: 'a'.repeat(32) } });
  fireEvent.click(screen.getByRole('button')); expect(await screen.findByRole('alert')).toHaveTextContent('Envio falhou.');
  expect(screen.getByLabelText('Código da empresa')).toHaveValue('a'.repeat(32));
  expect(screen.queryByText('Inscrição confirmada e paga pela empresa.')).not.toBeInTheDocument();
});
